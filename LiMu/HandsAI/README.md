...
Train 和 Valid 和 Test 数据集的概念一定要十分清楚！可以通过一个泰勒展开的例子深入理解拟合的概念。尽量减少过拟合的方法可以分为正则化法（**通过限制模型的学习范围来控制模型的复杂性**），BN 法等。权重衰退和 Dropout 都是经典的正则化法。
...
注意只有在 Train 的时候 Dropout 层才有效果（引入随机性）在 Test 的时候全部都用到。
