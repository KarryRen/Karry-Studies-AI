...
一方面，在响应字段中只保留预测目标 `responder_6`，这样数据总字段数变为 84 个。另一方面，对剩余的每个字段进行数据类型规范化，通过修改数据类型以减少大约 48.6% 的占存（从 5150.65 MB 到 2647.53 MB）。
...
进行全局 z-score 归一化。注意：这个地方由于内存限制未采用循环 z-score 方法，只是对真实情况进行模拟。
